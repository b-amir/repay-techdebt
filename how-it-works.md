...
`scripts/**` are mechanically enforced.
...
Omnibus regex (title/focus/learnerOutcome): phrases like “whole app”, “entire codebase”, “complete overview”, “everything about”, etc. (`scripts/lib/curriculum-policy.js`).
...
| `scripts/lib/targeting.js` | Root resolution refusals |
| `scripts/lib/private-storage.js` | Private vs project-local locate |
| `scripts/lib/memory-paths.js` | `resolveMemoryPaths` / memory layout (CLI must not own this) |
| `scripts/lib/dialogue-envelope.js` | Envelope + `topicSignalClass` |
| `scripts/lib/program-model-schema.js` | Zod schemas + `MODEL_VERSION` |
| `scripts/lib/program-scan.js` | File discovery / classify helpers |
| `scripts/lib/program-coverage.js` | Pure coverage aggregation |
| `scripts/lib/plan-analysis-core.js` | `planAnalysis` + `summarizeModel` |
| `scripts/lib/program-intelligence.js` | Facade: `buildProgramModel` + re-exports |
| `scripts/lib/curriculum-planning.js` | Curriculum proposal |
| `scripts/lib/curriculum-approval.js` | Approval / corroboration / partial-scope gates |
| `scripts/lib/curriculum-policy.js` | Omnibus topic detection |
| `scripts/lib/approve-curriculum.js` | `validateCurriculum` (approval + structural floors) |
| `scripts/lib/claim-faithfulness.js` | CLAIMS parse + snippet overlap |
| `scripts/lib/lesson-citation-check.js` | Citation validity + shared extract |
| `scripts/lib/lesson-quality.js` | Mechanical lesson QA |
| `scripts/lib/save-lesson.js` | `evaluateLessonForSave` + `runTeachFloors` |
| `scripts/lib/analyzer-result.js` | Shared analyzer result factory (no base-class port) |
| `scripts/lib/trajectory.js` | Mode trajectories |
