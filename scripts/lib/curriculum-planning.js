...
function pathPenalty(path, kind) {
  let penalty = 0;
  if (NON_PRODUCT.test(path) && kind !== "test") penalty += 30;
  if (/\.(?:test|spec)\.[^.]+$/i.test(path) && kind !== "test") penalty += 22;
  if (
    /package(?:-lock)?\.json|pnpm-lock|yarn\.lock|vite\.config|eslint|prettier|tsconfig/i.test(path)
  )
    penalty += 28;
  if (/(?:^|\/)(?:types?|constants?|styles?)(?:\.[^/]*)?$/i.test(path)) penalty += 10;
  if (/logo|(?:^|[/_-])icons?(?:[-_./]|$)|illustration|avatar/i.test(path)) penalty += 22;
  if (/^(?:app|src)\/(?:features?|domains?|modules?|routes?)$/i.test(path)) penalty += 15;
  if (/(?:^|\/)(?:components?|containers?|hooks?|utils?)$/i.test(path)) penalty += 10;
  if (/(?:^|\/)index\.[^/]+$/i.test(path) && kind !== "entry") penalty += 8;
  return penalty;
}
