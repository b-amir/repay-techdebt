...
export function evaluatePedagogy(markdown) {
  ...
  if (!/(?:common mistake|pitfall|warning|note|misconception|careful|instead of|rather than)/i.test(lowerMd)) {
  ...
  if (!hasChallenge) {
    ...
    if (!hasChallengeType) {
      warnings.push("Challenge should explicitly be a tracing, debugging, modification, or design task.");
    }
