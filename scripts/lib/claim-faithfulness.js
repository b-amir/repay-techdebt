  ...
  return [
    ...new Set(
      String(text)
        ...
        .replace(/[^a-z0-9_./-]+/g, " ")
        .split(/\s+/)
        .filter((token) => token.length >= 4 && !/^(this|that|with|from|into|when|then|than|have|been|will|your|their|about|which|where|what)$/.test(token)),
    ),
  ];
}
...
export async function assessClaimFaithfulness(targetRoot, markdown, options = {}) {
  ...
  if (explicit.length > 0) {
    ...
    for (const citation of citations.slice(0, options.maxAutoClaims ?? 5)) {
      ...
      assessments.push(assessment);
      if (assessment.citationMissing)
        problems.push(`Citation does not resolve: ${citation}`);
      else if (assessment.support === "no")
        problems.push(
          `Weak claim↔snippet overlap for "${claim.slice(0, 80)}…" @ ${citation}`,
        );
    }
  }
