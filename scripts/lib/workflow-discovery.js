...
export function discoverWorkflows(model) {
  const candidates = [];
  const routeNodes = model.nodes.filter(n => n.kind === "route" || n.name.toLowerCase().includes("route"));
  const commandNodes = model.nodes.filter(n => n.kind === "command" || n.name.toLowerCase().includes("cli") || n.name.toLowerCase().includes("command"));
