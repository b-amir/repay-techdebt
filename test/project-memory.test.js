...
test("discoverable workbook saves a curriculum first and links each lesson from INDEX.md", async () => {
  ...
  const environment = { REPAY_TECHDEBT_STATE_DIR: state, REPAY_TECHDEBT_CACHE_DIR: cache };
  try {
    await mkdir(target);
    await writeLessonEvidence(target);
    ...
    assert.match(await readFile(resolve(output, "INDEX.md"), "utf8"), /1 focused subject/);
