...
test("claim faithfulness passes golden lesson and fails unfaithful claims", async () => {
  ...
  try {
    ...
    await assert.rejects(
      ...
      (/** @type {any} */ error) => error.exitCode === 2 || error.code === 2,
    );
  } finally {
...
test("workbook trajectory stub validates ask fidelity order", () => {
  ...
  const result = validateFlow(stub);
  assert.equal(result.ok, true, result.errors ? result.errors.join("; ") : "");

  const missing = [
    FLOW_STATES.SETUP,
    FLOW_STATES.PURPOSE,
    FLOW_STATES.GATHER, // Invalid: setup -> purpose -> gather skips shortlist
  ];
  const failed = validateFlow(missing);
