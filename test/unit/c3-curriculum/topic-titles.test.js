...
test("titleFor is path label — unique, no chapter stamp openers", () => {
  ...
  const cases = [
    ...
    ["module", "app/server/request-adapter/body.server.ts"],
...
test("planned curriculum index has unique labels and no stamp openers", () => {
  ...
  const paths = [
    ...
    "app/server/request-adapter/body.server.ts",
    "app/server/request-adapter/headers.server.ts",
    "app/server/request-adapter/origin.server.ts",
