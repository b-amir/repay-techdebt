...
test("focused planning changes relevance without claiming enhanced tools ran", async () => {
  ...
  try {
    ...
    const plan = analysisPlanSchema.parse(
      planAnalysis(model, {
        mode: "focused",
        focus: "security authorization auth",
      }),
    );
...
test("discovers monorepo components and AST-observed dynamic and test relationships", async () => {
  ...
  try {
    await mkdir(resolve(directory, "packages", "api", "src"), {
      recursive: true,
    });
    await mkdir(resolve(directory, "packages", "ui", "src"), {
      recursive: true,
    });
    await mkdir(resolve(directory, "packages", "api", "tests"), {
      recursive: true,
    });
    await writeFile(
      resolve(directory, "package.json"),
...
test("Elixir relations, configuration, tests, and runtime candidates remain visible", async () => {
  ...
  try {
    await mkdir(resolve(directory, "lib", "sample", "auth"), {
      recursive: true,
    });
    await mkdir(resolve(directory, "test", "sample", "auth"), {
      recursive: true,
    });
