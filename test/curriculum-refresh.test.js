...
import { computeEvidenceDigests, refreshCurriculum } from "../scripts/lib/curriculum-refresh.js";
...
test("refreshCurriculum detects stale lessons based on evidence file changes", async () => {
  ...
  try {
    ...
    assert.equal(curriculum.topics[0].status, "stale");
    assert.deepEqual(curriculum.topics[0].staleReasons, ["Evidence deleted or moved: src/example.ts"]);
